...
public class Rotating90 {
	...
	static void RemoveResources(BlockPlaceEvent event) {
		if (!Functions.CheckBlockMaterialLists(MaterialLists.Doors)) {
			...
			java.util.ArrayList<ItemStack> doors = new java.util.ArrayList<ItemStack>();
			ItemStack[] arrayOfItemStack; int j = (arrayOfItemStack = event.getPlayer().getInventory().getContents()).length; for (int i = 0; i < j; i++) {
