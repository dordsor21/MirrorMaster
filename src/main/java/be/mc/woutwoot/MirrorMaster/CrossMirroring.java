...
public class CrossMirroring {
    ...
    static void RemoveResources(BlockPlaceEvent event) {
        if (!Functions.CheckBlockMaterialLists(MaterialLists.Doors)) {
            ...
            collectDoors(event, doors, arrayOfItemStack, j);
            ...
            if ((Variables.xDif != 0) || (Variables.zDif != 0)) {
                ...
                if ((Variables.xDif == 0) || (Variables.zDif == 0)) {
                    ...
                    if (doDoors(event, doors, handItem)) return;
                }
            }
    ...
    static void collectDoors(BlockPlaceEvent event, ArrayList<ItemStack> doors, ItemStack[] arrayOfItemStack, int j) {
        for (int i = 0; i < j; i++) {
    ...
    static boolean doDoors(BlockPlaceEvent event, ArrayList<ItemStack> doors, int handItem) {
        if (doors.size() >= 4) {
            event.getPlayer().getInventory().clear(handItem);
            event.getPlayer().getInventory().clear(event.getPlayer().getInventory().first(doors.get(0)));
            event.getPlayer().getInventory().clear(event.getPlayer().getInventory().first(doors.get(0)));
